...
export function useFrameExtraction({ targetFrames }: { targetFrames: number }) {
  ...
  const handleSelect = useCallback(async (file: File) => {
    ...
    try {
      ...
      const duration = next.durationMs / 1000
      const loopLength = duration > 0 ? Math.min(2, duration) : 0
      ui.trim = [0, loopLength]
      settings.committedTrim = [0, loopLength]
      ui.view = [0, duration > 0 ? duration : 1]
  ...
  useEffect(() => {
    ...
    if (prev !== frames) {
      closeFrames(prev, new Set(frames))
    }
  }, [frames])
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const duration = sampler.durationMs / 1000
    const count = 24
    const times = Array.from({ length: count }, (_, index) => ((index + 0.5) / count) * duration)
    ...
    sampler
      ...
      .then((bitmaps) => {
        ...
        const next = times.map((time, index) => ({ time, bitmap: bitmaps[index] }))
    ...
    return () => {
      ...
      for (const thumb of timelineRef.current) {
        thumb.bitmap.close()
      }
  ...
  const requestFrame = useCallback(
    async (time: number) => {
      ...
      const [bitmap] = await sampler.sampleAtTimes([time], { maxDecodeSize: 200 })
  ...
  const runLivePreview = useCallback(
    (inTime: number, outTime: number) => {
      if (!sampler || durationSec <= 0) {
        return
      }

      if (liveBusy.current) {
        liveQueued.current = [inTime, outTime]
        return
      }
      ...
      sampler
        .sampleAtTimes([inTime, outTime], { maxDecodeSize: 220 })
        .then(([inBitmap, outBitmap]) => {
          ...
          inFrameRef.current = inBitmap ?? null
          setInFrame(inBitmap ?? null)
          outFrameRef.current?.close()
          outFrameRef.current = outBitmap ?? null
          setOutFrame(outBitmap ?? null)
        })
        .catch(() => {})
        .finally(() => {
